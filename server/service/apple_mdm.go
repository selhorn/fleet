...
func (svc *MDMAppleCheckinAndCommandService) handleDeviceNameCommandResult(ctx context.Context, cmdResult *mdm.CommandResults) error {
	...
	if status == fleet.MDMAppleStatusAcknowledged {
		...
		if err := svc.ds.UpdateHostDeviceNameStatusFromCommand(ctx, cmdResult.CommandUUID, fleet.MDMDeliveryVerifying, ""); err != nil && !fleet.IsNotFound(err) {
			return ctxerr.Wrap(ctx, err, "update device name row from acknowledged command")
		}
		return nil
	}

	if err := svc.ds.UpdateHostDeviceNameStatusFromCommand(ctx, cmdResult.CommandUUID, fleet.MDMDeliveryFailed, detail); err != nil && !fleet.IsNotFound(err) {
...
func (svc *MDMAppleCheckinAndCommandService) handleRefetchDeviceResults(ctx context.Context, host *fleet.Host, cmdResult *mdm.CommandResults) (*mdm.Command, error) {
	...
	if deviceNameOK {
		...
		if err := svc.ds.VerifyHostDeviceName(ctx, host.UUID, deviceName); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "verify host device name from refetch")
		}
	}
